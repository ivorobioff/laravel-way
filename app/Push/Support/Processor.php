...
use Log;
use Exception;
use RealEstate\Support\Chance\Attempt;
use RealEstate\Support\Chance\LogicHandlerInterface;
...
class Processor implements LogicHandlerInterface
{
	...
	public function process(Payload $payload, callable $onError = null)
	{
        $calls = array_map(function(Call $call){
            return ['url' => $call->getUrl(), 'secret1' => $call->getSecret1(), 'secret2' => $call->getSecret2()];
        }, $payload->getCalls());
    ...
    {
        $tunnel = new Tunnel($call['url'], $call['secret1'], $call['secret2']);
